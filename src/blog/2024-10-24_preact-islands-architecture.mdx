...
need some client-side Javascript to work.

At which point, we only `hydrate` (download Javascript) specific components that
require some type of user interactivity, while the other static regions of the
page are pure non-interactive HTML that do not require hydration.
...
What be want is a way to allow specific components to be rendered on the server
as placeholders and then hydrated into interactive elements on the client.

Two key functions make this happen:

```tsx
// src/lib/preact-islands.tsx
import { FunctionalComponent, JSX, hydrate } from "preact";
...
This is where most of the magic happens, basically this function registers a
...
Now we need to configure Vite to avoid sending the entire app's code to the
browser. instead we only want to send the islands. Fortunately, we can do so via
